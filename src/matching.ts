...
function spatialMatchHelper(
  ...
  while (i < password.length - 1) {
    ...
    let turns = 0;
    const isQwertyOrDvorak =
      graphName === "qwerty" || graphName === "dvorak";
...
export function repeatMatch(password: string): RepeatMatch[] {
  ...
  while (lastIndex < password.length) {
    ...
    const baseAnalysis = mostGuessableMatchSequence(
      baseToken,
      omnimatch(baseToken)
    );
...
export function regexMatch(
  password: string,
  regexen: Record<string, RegExp> = REGEXEN
): RegexMatch[] {
