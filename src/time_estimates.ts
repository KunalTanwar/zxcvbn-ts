...
export function estimateAttackTimes(guesses: number): AttackTimes {
  ...
  for (const [scenario, rate] of Object.entries(ATTACK_RATES) as Array<
    [ScenarioKey, number]
  >) {
    (crackTimesSeconds as Record<ScenarioKey, number>)[scenario] =
      guesses / rate;
  }

  const crackTimesDisplay = {} as CrackTimesDisplay;
  for (const [scenario, seconds] of Object.entries(crackTimesSeconds) as Array<
    [ScenarioKey, number]
  >) {
    (crackTimesDisplay as Record<ScenarioKey, string>)[scenario] =
      displayTime(seconds);
  }
